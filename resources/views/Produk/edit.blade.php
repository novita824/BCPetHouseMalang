        ...
        <div class="main-panel">
          <div class="content-wrapper">
            <div class="page-header">
              <h3 class="right" class="page-title"> EDIT DATA PRODUK </h3>
            </div>

        <div class="row">
          <div class="col-lg-7 mt-5 mt-lg-0 d-flex align-items-stretch">
            <form action="{{ route('update.produk') }}" method="POST" class="php-email-form">
              @csrf
              <div class="row">
                <input type="hidden" name="id" value="{{ $pk->id }}">
                <div class="form-group">
                  <label for="name">Nama</label>
                  <input type="text" class="form-control" value="{{ $pk->nama }}" name="nama" id="subject" required>
                </div>
                <div class="form-group">
                  <label for="name">Jumlah</label>
                  <input type="text" class="form-control" value="{{ $pk->jumlah }}" name="jumlah" id="subject" required>
                </div>
              <div class="form-group">
                <label for="name">Harga</label>
                <input type="text" class="form-control" value="{{ $pk->harga }}" name="harga" id="subject" required>
              </div>
              <div class="form-group">
                <label for="name">Harga Pokok</label>
                <input type="text" class="form-control" value="{{ $pk->hargapokok }}" name="hargapokok" id="subject" required>
              </div>
              <div class="form-group">
                <label for="name">Total</label>
                <input type="text" class="form-control" value="{{ $pk->total }}" name="total" id="subject" required>
              </div>
